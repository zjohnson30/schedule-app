...
const DEFAULT_TASKS = [
  { id: "demo1", title: "Morning workout", time: "07:00", category: "Health", priority: "high", completed: false, date: toLocalDateStr(today), notes: "" },
  { id: "demo2", title: "Team standup", time: "09:30", category: "Work", priority: "medium", completed: false, date: toLocalDateStr(today), notes: "Prepare sprint update" },
  { id: "demo3", title: "Grocery shopping", time: "17:00", category: "Errands", priority: "low", completed: false, date: toLocalDateStr(today), notes: "" },
];
...
export default function ScheduleTracker() {
  ...
  // Save tasks whenever they change (skip initial load)
  useEffect(() => {
    if (loading) return;
  ...
  const filteredTasks = tasks.filter(t => {
    const matchDate = view === "day"
      ? t.date === dateStr
      : getWeekDates().some(d => t.date === toLocalDateStr(d));
    ...
    <div style={styles.container}>
      ...
      <div style={styles.navBar}>
        <div style={styles.navLeft}>
          <button style={styles.navArrow} onClick={() => navigateDate(-1)}>‹</button>
      ...
      <div style={styles.weekStrip}>
        {getWeekDates().map((d, i) => {
          ...
          const hasTasks = tasks.some(t => t.date === toLocalDateStr(d));
          return (
            ...
            <div key={task.id}
              ...
              <div style={styles.taskLeft}>
                <button onClick={() => toggleComplete(task.id)} style={styles.checkbox}>
                  {task.completed && (
                    <span style={{ ...styles.checkmark, animation: "checkPop 0.3s ease" }}>✓</span>
                  )}
                </button>
                <div>
                  ...
                  <div style={styles.taskMeta}>
                    {task.time && <span style={styles.taskTime}>⏰ {task.time}</span>}
              ...
              <div style={styles.taskActions}>
                <button style={styles.actionBtn} onClick={() => { setEditTask(task); setShowAddModal(true); }}>✏️</button>
                <button style={styles.actionBtn} onClick={() => deleteTask(task.id)}>🗑️</button>
              </div>
            </div>
          ))
        )}
      ...
      {showAddModal && (
        <TaskModal
          ...
          onSave={(task) => {
            if (editTask) {
              setTasks(prev => prev.map(t => t.id === editTask.id ? { ...editTask, ...task } : t));
            } else {
...
function TaskModal({ task, defaultDate, onSave, onClose }) {
  ...
  const handleSubmit = () => {
    ...
    onSave({ title: title.trim(), time, category, priority, date, notes: notes.trim() });
  };
